...
from gateway_config import load_gateway_config, require_gateway_auth
from auto_recall import auto_recall, build_memory_context
from memory_injector import inject_into_anthropic, inject_into_openai, anthropic_messages_to_openai_messages
from llm_clients import call_anthropic, call_openai_compatible
...
async def health_check(request):
    ...
    try:
        ...
        return JSONResponse({
            ...
            "gateway": "enabled" if gateway_config.enabled else "disabled",
            "gateway_mock_llm": gateway_config.mock_llm,
        })
    except Exception as e:
...
# =============================================================
# Gateway endpoints — automatic memory recall + model proxy
# 网关接口 —— 自动召回记忆并转发给主模型
# =============================================================
async def _gateway_recall_for_messages(messages, body):
    if not gateway_config.auto_recall_enabled or body.get("memory", {}).get("enabled", True) is False:
        return {"query": "", "memory_context": "", "raw_recall": "", "used": False, "token_estimate": 0}
    memory_opts = body.get("memory", {}) if isinstance(body.get("memory", {}), dict) else {}
    return await auto_recall(
        messages=messages or [],
        breath_func=breath,
        recent_turns=int(memory_opts.get("recent_turns", gateway_config.recent_turns)),
        max_memories=int(memory_opts.get("max_memories", gateway_config.max_memories)),
        max_tokens=int(memory_opts.get("max_tokens", gateway_config.max_tokens)),
        include_related=bool(memory_opts.get("include_related", gateway_config.include_related)),
        include_floating=bool(memory_opts.get("include_floating", gateway_config.include_floating)),
        debug=bool(memory_opts.get("debug", gateway_config.debug) or body.get("debug", False)),
    )


@mcp.custom_route("/api/auto-recall", methods=["POST"])
async def api_auto_recall(request):
    from starlette.responses import JSONResponse
    if not require_gateway_auth(request.headers, gateway_config):
    ...
    try:
        ...
        return JSONResponse({"error": "Invalid JSON"}, status_code=400)
    try:
        result = await _gateway_recall_for_messages(body.get("messages") or [], body)
        return JSONResponse(result)
    except Exception as e:
        logger.exception("auto-recall failed")
        return JSONResponse({"error": str(e)}, status_code=500)


@mcp.custom_route("/v1/messages", methods=["POST"])
async def anthropic_messages_gateway(request):
    from starlette.responses import JSONResponse
    if not require_gateway_auth(request.headers, gateway_config):
        return JSONResponse({"error": {"type": "authentication_error", "message": "Unauthorized"}}, status_code=401)
    if not gateway_config.enabled:
        return JSONResponse({"error": {"type": "gateway_disabled", "message": "Gateway disabled"}}, status_code=503)
    try:
        body = await request.json()
    except Exception:
        return JSONResponse({"error": {"type": "invalid_request_error", "message": "Invalid JSON"}}, status_code=400)
    if body.get("stream") is True:
        return JSONResponse({"error": {"type": "unsupported", "message": "Streaming is not supported in first-pass gateway yet"}}, status_code=400)
    try:
        recall = await _gateway_recall_for_messages(body.get("messages") or [], body)
        payload = inject_into_anthropic(body, recall.get("memory_context", ""))
        if gateway_config.main_model_name and not payload.get("model"):
            payload["model"] = gateway_config.main_model_name
        response = await call_anthropic(payload, gateway_config, memory_used=recall.get("used", False))
        if body.get("memory", {}).get("debug") or body.get("debug"):
            response["ombre_memory"] = recall
        return JSONResponse(response)
    except httpx.HTTPStatusError as e:
        return JSONResponse({"error": "upstream_error", "detail": e.response.text}, status_code=e.response.status_code)
    except Exception as e:
        logger.exception("anthropic gateway failed")
        return JSONResponse({"error": "gateway_error", "detail": str(e)}, status_code=500)


@mcp.custom_route("/v1/chat/completions", methods=["POST"])
async def openai_chat_completions_gateway(request):
    from starlette.responses import JSONResponse
    if not require_gateway_auth(request.headers, gateway_config):
        return JSONResponse({"error": {"message": "Unauthorized", "type": "authentication_error"}}, status_code=401)
    if not gateway_config.enabled:
        return JSONResponse({"error": {"message": "Gateway disabled", "type": "gateway_disabled"}}, status_code=503)
    try:
        body = await request.json()
    except Exception:
        return JSONResponse({"error": {"message": "Invalid JSON", "type": "invalid_request_error"}}, status_code=400)
    if body.get("stream") is True:
        return JSONResponse({"error": {"message": "Streaming is not supported in first-pass gateway yet", "type": "unsupported"}}, status_code=400)
    try:
        recall = await _gateway_recall_for_messages(body.get("messages") or [], body)
        payload = inject_into_openai(body, recall.get("memory_context", ""))
        if gateway_config.main_model_name and not payload.get("model"):
            payload["model"] = gateway_config.main_model_name
        response = await call_openai_compatible(payload, gateway_config, memory_used=recall.get("used", False))
        if body.get("memory", {}).get("debug") or body.get("debug"):
            response["ombre_memory"] = recall
        return JSONResponse(response)
    except httpx.HTTPStatusError as e:
        return JSONResponse({"error": {"message": e.response.text, "type": "upstream_error"}}, status_code=e.response.status_code)
    except Exception as e:
        logger.exception("openai gateway failed")
        return JSONResponse({"error": {"message": str(e), "type": "gateway_error"}}, status_code=500)


@mcp.custom_route("/chat", methods=["POST"])
async def custom_chat_gateway(request):
    from starlette.responses import JSONResponse
    if not require_gateway_auth(request.headers, gateway_config):
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    try:
        ...
        return JSONResponse({"error": "Invalid JSON"}, status_code=400)
    provider = (body.get("provider") or gateway_config.main_provider or "anthropic").lower()
    messages = body.get("messages") or []
    system = body.get("system", "")
    try:
        if provider == "anthropic":
            payload = {k: v for k, v in body.items() if k not in {"provider", "memory", "debug"}}
            payload.setdefault("messages", messages)
            if system:
                payload.setdefault("system", system)
            payload.setdefault("model", body.get("model") or gateway_config.main_model_name or "claude-3-5-sonnet-latest")
            payload.setdefault("max_tokens", body.get("max_tokens", 1024))
            recall = await _gateway_recall_for_messages(messages, body)
            payload = inject_into_anthropic(payload, recall.get("memory_context", ""))
            response = await call_anthropic(payload, gateway_config, memory_used=recall.get("used", False))
        else:
            payload = {k: v for k, v in body.items() if k not in {"provider", "memory", "debug", "system"}}
            payload["messages"] = anthropic_messages_to_openai_messages(system, messages)
            payload.setdefault("model", body.get("model") or gateway_config.main_model_name or "gpt-4o-mini")
            recall = await _gateway_recall_for_messages(payload.get("messages") or [], body)
            payload = inject_into_openai(payload, recall.get("memory_context", ""))
            response = await call_openai_compatible(payload, gateway_config, memory_used=recall.get("used", False))
        return JSONResponse({"response": response, "memory": recall if body.get("memory", {}).get("debug") or body.get("debug") else {"used": recall.get("used", False)}})
    except Exception as e:
        logger.exception("custom chat gateway failed")
        return JSONResponse({"error": str(e)}, status_code=500)
...
_GATEWAY_PUBLIC_ENV_FIELDS = [
    ...
    "OMBRE_EMBEDDING_DIMENSIONS",
]

_GATEWAY_SECRET_ENV_FIELDS = {
...
def _gateway_env_snapshot() -> dict:
    fields = list(_GATEWAY_PUBLIC_ENV_FIELDS)
    for secret_name in ("OMBRE_MAIN_MODEL_API_KEY", "OMBRE_API_KEY", "OMBRE_EMBEDDING_API_KEY", "OMBRE_GATEWAY_AUTH_TOKEN"):
        if secret_name not in fields:
            fields.append(secret_name)
    values = {}
    for name in fields:
        raw = _read_env_var(name)
        values[name] = {
            "value": "" if name in _GATEWAY_SECRET_ENV_FIELDS else raw,
            "masked": _mask_secret(raw) if name in _GATEWAY_SECRET_ENV_FIELDS else "",
            "source": "env" if os.environ.get(name, "").strip() else ("file" if raw else ""),
            "secret": name in _GATEWAY_SECRET_ENV_FIELDS,
        }
    return values


@mcp.custom_route("/api/gateway-config", methods=["GET"])
async def api_gateway_config_get(request):
    """Read gateway/model env config for Dashboard. Secret values are masked only."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err: return err
    return JSONResponse({
        ...
        "values": _gateway_env_snapshot(),
        "restart_command": "cd /opt/ombre-memory-gateway/app && docker compose -f docker-compose.gateway.yml restart",
    })


@mcp.custom_route("/api/gateway-config", methods=["POST"])
async def api_gateway_config_set(request):
    """Persist gateway/model env config to .env. Blank secrets mean keep old value."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err: return err
    try:
    ...
    allowed = set(_GATEWAY_PUBLIC_ENV_FIELDS) | _GATEWAY_SECRET_ENV_FIELDS
    updated = []
    skipped = []
    for name, raw in values.items():
        ...
        if raw is None:
            continue
        value = str(raw).strip()
        if name in _GATEWAY_SECRET_ENV_FIELDS and value == "":
            continue
        if "\n" in value or "\r" in value or '"' in value or "'" in value:
            return JSONResponse({"error": f"{name} must not contain quotes or newlines"}, status_code=400)
        try:
            _write_env_var(name, value)
            updated.append(name)
        except Exception as e:
            return JSONResponse({"error": f"failed to write {name}: {e}", "updated": updated}, status_code=500)
